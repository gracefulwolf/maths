...
var PAGE_DEPTH = { vi: 0, pi: 1, ci: 2, si: 2, ti: 0 };
...
function initPage() {
	...
	$(".quiz-check.image-answer").on("quizCheck", function(e, params){
		...
		var nextQuiz = $("#img-quiz-0" + (idx));
		console.log(nextQuiz);
		nextQuiz.find(".check-item").removeClass("hide");
	});

	$(".quiz-content").on("QUIZ_RESET_ANSWER", function(){
		$(this).find(".quiz-check.image-answer .check-item").addClass("hide");
		$("#img-quiz-01").find(".check-item").removeClass("hide");
	})
};
