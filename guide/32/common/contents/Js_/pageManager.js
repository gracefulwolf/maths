...
$ts.loadScriptFile("../common/libs/js/polyfills.js"), function () {
    var t = $ts.ce({tag: "div", class: "loadingImage", parent: document.body}), e = setInterval(function () {
    ...
    window.parent == window ? (console.log("[  Run Contents Mode(no UI)  ]"), document.querySelector("html").classList.add("noUI"), $ts.loadScriptFile("../common/libs/js/scale.js", function () {
        ...
        t.preventDefault()
    }, {passive: !1}), window.$run = function (t, e) {
        var n = setInterval(function () {
            "" !== t && void 0 === window[t] || (clearInterval(n), e && e())
        }, 10)
    }, window.$callBack = {};
    var g = {}, f = {
        inPage: {slider: [], quiz: [], scroll: []},
        inPopup: {slider: [], quiz: [], scroll: []},
        popup: [],
        controller: [],
        singleAudio: [],
        sync: [],
    ...
    function p(n) {
        ...
        e.slideClass = n.slideClass ? n.slideClass : "on", e.tabClass = n.tabClass ? n.tabClass : "on", e.btnClass = n.btnClass ? n.btnClass : "off", e.callbacks = {
            ...
            loop: {left: "left" === n.carousel.loop, right: "right" === n.carousel.loop},
        ...
        this.btn = i.btn, this.quizArray = [], this.completeCount = 0, this.toggleBtn = function () {
            ...
            for (var e in t) !function (n) {
                var a = setInterval(function () {
                    if (0 !== f[i.include].quiz.length) for (var t in f[i.include].quiz) {
                        var e;
    ...
    function v(t) {
        var e = this, n = t.targets, a = t.btn, i = (t.idx, t.callBack), o = [];
    ...
    function n() {
        ...
        if ($ts.getEl("[data-popup-btn]") && $ts.getEl("[data-popup-container]") && function (t, e) {
            ...
            for (var i in setTimeout(function () {
                ...
                var o = t[i].getAttribute("data-popup-btn"), r = $ts.getEl('[data-popup-page="' + o + '"]')[0],
                    ...
                    d = !!t[i].hasAttribute("data-close-inner"), u = !!parent.document.querySelector("iframe"),
                    p = $popup({
                        btn: t[i],
                        page: r,
                        popupPages: n,
                        ...
                        callBack: {
                            open: function (t) {
                                var e;
                                if (t.container.hasAttribute("data-slide-container")) for (var n in t.container.classList.add("on"), f.inPopup.slider) f.inPopup.slider[n].container === t.container && f.inPopup.slider[n].reset({idx: t.idx - 1});
            ...
            for (var e in t) {
                var n = t[e].getAttribute("data-media-container"), a = $ts.getEl("[data-media-btn]", t[e]),
                    i = t[e].getAttribute("data-media-control"), o = new $controller({
                        container: t[e],
                        mediaType: n,
                        controlType: i,
                        mediaBtns: a,
            ...
            for (var a in n) !function (t) {
                for (var e = t.getAttribute("data-slide-container"), n = t.children, a = null, i = null, o = null, r = null, s = null, l = {}, c = 0; c < n.length; c++) if (n[c].hasAttribute("data-slides") && (a = n[c].children), n[c].hasAttribute("data-tabs-toggle") && (s = n[c]), n[c].hasAttribute("data-slide-btn-container")) for (var d = n[c].children, u = 0; u < d.length; u++) d[u].hasAttribute("data-tabs") && (i = d[u].children), d[u].hasAttribute("data-btn-prev") && (o = d[u]), d[u].hasAttribute("data-btn-next") && (r = d[u]); else n[c].hasAttribute("data-tabs") && (i = n[c].children), n[c].hasAttribute("data-btn-prev") && (o = n[c]), n[c].hasAttribute("data-btn-next") && (r = n[c]);
        ...
        ($ts.getEl("[data-scroll-container]") || $ts.getEl("[data-scroll-popup-container]")) && function (t) {
            var e = {};
            for (var n in e.inPage = t.page, e.inPopup = t.popup, e) for (var a in e[n]) {
                var i = e[n][a], o = $ts.getEl("[data-scroll-inner]", e[n][a], 0),
                    r = $ts.getEl("[data-scroll-content]", e[n][a], 0), s = e[n][a].getAttribute("data-scroll-reset"),
                    l = e[n][a].getAttribute("data-scroll-value"), c = e[n][a].getAttribute("data-scroll-container");
                "all" === c ? (d({
                    container: i,
                    inner: o,
                    content: r,
                    ...
                    value: l,
                    type: "horizon"
                }, n)) : d({container: i, inner: o, content: r, reset: s, value: l, type: c}, n)
            }
        ...
        var i, o, r = $ts.getEl("main [data-quiz]"), s = $ts.getEl("[data-popup-container] [data-quiz]"),
            l = $ts.getEl("main [data-quiz-chain]"), c = $ts.getEl("[data-popup-container] [data-quiz-chain]");
        for (var a in r) {
            var d = r[a].getAttribute("data-quiz");
            $quiz({container: r[a], type: d, include: "inPage"})
        }
        for (var a in s) {
            d = s[a].getAttribute("data-quiz");
            $quiz({container: s[a], type: d, include: "inPopup"})
        }
        for (var a in l) new m({btn: l[a], include: "inPage"});
        for (var a in c) new m({btn: c[a], include: "inPopup"});
    ...
    g.inPageReset = function () {
        for (var t in g.mediaReset(), g.popupClose(), f.inPage) if (f.inPage[t].length) for (var e in f.inPage[t]) g.reset(f.inPage[t][e], t);
        window.resetAppears()
    }, g.inPopupReset = function () {
        for (var t in f.inPopup) if (f.inPopup[t].length) for (var e in f.inPopup[t]) g.reset(f.inPopup[t][e], t);
        ...
        if (f.popup.length) for (var t in f.popup) f.popup[t].close()
    }, g.mediaReset = function () {
        if (f.controllerLen) for (var t in f.controller) f.controller[t].stop();
        if (f.singleAudioLen) for (var t in f.singleAudio) f.singleAudio[t].stop();
        if (f.sync.length) for (var t in f.sync) f.sync[t].reset()
