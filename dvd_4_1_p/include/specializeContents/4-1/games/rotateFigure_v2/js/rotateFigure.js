function initScene() {
  ...
  for (var i = 0; i < gameManager.QUIZ_ANSWER.length; i++) {
    ...
    var dropAreaTop = 128;
    var dropAreaLeft = i ? 780 : 520;

    if (gameManager.QUIZ_OPTION[0][1] === 2) {
      dropAreaTop -= 3;
      dropAreaLeft += 4;
    } else if (gameManager.QUIZ_OPTION[0][1] === 3) {
      dropAreaTop -= 3;
      dropAreaLeft += 2;
    } else if (gameManager.QUIZ_OPTION[0][1] === 4) {
      dropAreaTop -= 23;
      dropAreaLeft = i ? 774 : 508;
    } else if (gameManager.QUIZ_OPTION[0][1] === 5) {
      dropAreaTop += i ? -3 : 2;
      dropAreaLeft += 2;
    }

    // if (isImgMode) {
    //   dropArea.setAttribute("style", "background: url(images/flatFigure_questionBox_" + 5 + ".png)");
    // } else {
...
function appendSelectQuestion(buttonType, choiceQuestionArray, imgSrcArray) {
  ...
  for (var i = 0; i < gameManager.QUIZ_OPTION.length - 2; i++) {
    ...
    dragImg.setAttribute("src", "images/flatFigure_" + gameManager.QUIZ_OPTION[0][0] + "_" + gameManager.QUIZ_OPTION[0][1] + "." + imgExt);
