...
function appendSelectQuestion(buttonType, choiceQuestionArray, imgSrcArray) {
  ...
  for (var i = 0; i < gameManager.quizObj.length; i++) {
    ...
    var imgUrl = "../include/images/common/" + gameManager.quizObj[i][0] + "/";
    imgUrl += gameManager.quizObj[i][1];

    if (!/svg/g.test(gameManager.quizObj[i][1])) {
      imgUrl += ".png";
    }

    dragObj.style.background = "url(" + imgUrl + ")";
    dragObj.style.backgroundSize = "cover";
    if (i % 2 == 0) {
      dragObjTop = dragObjTop + 180;
      dragObjLeft = 930;
      if (intX == 1) {
      ...
      dragObjRightTop = dragObjRightTop + 180;
