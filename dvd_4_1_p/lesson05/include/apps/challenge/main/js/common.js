...
(function ($){
	...
	{
		function initFn() {
			if(this.options.drag) this.bg.on(touchstart, $.proxy(onStart, this));
			// ----------------------------------------------- jj 뷰어 스와이프 중복 처리
		...
		function onStart(e) {
			...
			// this.bg.on(touchmove + this.eventId, $.proxy(onMove, this));
		...
		function onEnd(e) {
			...
			// this.bg.off(touchmove + this.eventId);
