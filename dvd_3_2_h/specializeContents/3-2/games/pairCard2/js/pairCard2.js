function setGameSectionBackground(willOn) {
  parent.document.querySelector("section.game").style.background = willOn ? "url(./games/pairCard2/images/bgImg.png) center / cover no-repeat" : "";
}
...
function createObject(index, eventCallback, parentObjSrc) {
  ...
  if (gameManager.cardArray[index][0] == "clock") {
    cardBox.innerHTML =
      '<div class="cardImgC" style="background: url(../include/images/common/' +
      gameManager.cardArray[index][0] +
      "/" +
      gameManager.cardArray[index][1] +
      "_" +
      gameManager.cardArray[index][2] +
      "_" +
      gameManager.cardArray[index][3] +
      '.png); width: 240px; height: 243px; background-size: cover;  margin: 1px 0 0 19px;"></div>';
    ...
    // 	cardBox.innerHTML = '<div class="cardImgC" style="background: url(../include/images/common/' + gameManager.cardArray[index][0] + '/' + gameManager.cardArray[index][1] + '_' + gameManager.cardArray[index][2] + '_' + gameManager.cardArray[index][3] + '.png); width: 120px; height: 120px; background-size: cover;  margin: 60px 80px;"></div>';
    // } else {
    // 	cardBox.innerHTML = '<div class="cardImgC" style="background: url(../include/images/common/' + gameManager.cardArray[index][0] + '/' + gameManager.cardArray[index][1] + '_' + gameManager.cardArray[index][2] + '.png); width: 120px; height: 120px; background-size: cover;  margin: 60px 80px;"></div>';
    // }
    cardBox.innerHTML =
      '<div class="cardImgC" style="background: url(../include/images/common/' +
      gameManager.cardArray[index][0] +
      "/" +
      gameManager.cardArray[index][1] +
      '.png); width: 120px; height: 120px; background-size: cover;  margin: 60px 80px;"></div>';
  }
  ...
  cardBox.addEventListener('mousedown', eventCallback, false);
  // cardBox.addEventListener('touchstart', eventCallback, false);
...
function gameOver() {
  ...
  setTimeout(function () {
    ...
    setTimeout(function () {
      setGameSectionBackground(false);
    }, 1000);
  }, 1800);
}
