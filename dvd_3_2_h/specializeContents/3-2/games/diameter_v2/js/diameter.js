...
function initScene() {
  ...
  // 배경 변경
  if (IMAGE_SOURCES[0] === "42-5-07" || IMAGE_SOURCES[0] === "i32-6-02" || IMAGE_SOURCES[0] === "i32-6-05") {
  ...
  for (var i = 0, dropPositions, dropArea, answerValue; i < DROP_POSITIONS_ARRAY.length; i++) {
    ...
    dropArea.style.backgroundSize = "100% 100%";
    dropArea.style.backgroundImage = "url(./images/diameter_dragBox.png)";
...
function appendSelectQuestion(buttonType, choiceQuestionArray, imgSrcArray) {
  ...
  for (var i = 0, dragObj, height, dragPositions, left, top; i < choiceQuestionArray.length; i++) {
    ...
    dragObj.style.backgroundSize = "100% 100%";
...
function createTable(parentElement) {
  ...
  var titleInnerElement = document.createElement("div");
  ...
  titleInnerElement.style.boxSizing = "border-box";
  titleInnerElement.style.padding = "0 20px";
  titleInnerElement.style.background = "white";
  titleInnerElement.style.borderRadius = "8px";
  titleInnerElement.style.boxShadow = "rgba(0, 0, 0, 0.2) 0px -3px 0px inset";
  titleInnerElement.innerHTML = TABLE_TITLE;
