var isAnswerLength_3;

function getDragObjects() {
  return document.querySelectorAll(".choiceQuestionTxT");
}

function changeDragObjects() {
  if (!isAnswerLength_3) {
    return;
  }

  var dragObjects = getDragObjects();
  for (var i = 0, dragObject, len = dragObjects.length; i < len; i++) {
    dragObject = dragObjects[i];

    dragObject.style.letterSpacing = "16px";
    dragObject.style.textAlign = "right";
    dragObject.style.width = "170px";
    dragObject.style.padding = "0";
    if (dragObject.parentNode.classList.contains("correct")) {
      dragObject.style.marginLeft = "15px";
    }
  }
}
...
function updateStatics() {
  if (isAnswerLength_3) {
    var endNum = document.querySelector("#endNum");
    var center = document.querySelector("#center");

    if (endNum) {
      endNum.style.left = "383px";
      endNum.style.letterSpacing = "16px";
    }

    if (center) {
      center.style.left = "383px";
      center.style.letterSpacing = "16px";
    }
  }
}

function initScene() {
  isAnswerLength_3 = gameManager.TOTAL_ANSWER_ARRAY[1].toString().length === 3 || gameManager.TOTAL_ANSWER_ARRAY[2].toString().length === 3;
  // log('initScene...');
  ...
  console.log(gameManager.TOTAL_ANSWER_ARRAY);
...
function inithangerBar() {
  ...
  document.querySelector("#divisionTaste").style.width = "195px";
  document.querySelector("#divisionTaste").style.height = "79px";
  document.querySelector("#question2").style.width = "190px";

  if (gameManager.CURRENT_ANSWER[0].toString().length === 2 || gameManager.CURRENT_ANSWER[1].toString().length === 2) {
    document.querySelector("#robot2Dapbox_2_1").style.width = "124px";
    document.querySelector("#robot2Dapbox_2_1").style.height = "92px";
    document.querySelector("#robot2Dapbox_2_1").style.marginLeft = "48px";
    document.querySelector("#robot2Dapbox_2_2").style.width = "124px";
    document.querySelector("#robot2Dapbox_2_2").style.height = "92px";
    document.querySelector("#robot2Dapbox_2_2").style.marginLeft = "48px";

    var txts = document.querySelectorAll(".circle > #robotTXT");
    var txtsLen = txts.length;

    for (var i = 0, txt; i < txtsLen; i++) {
      txt = txts[i];
      txt.style.left = "48px";
      txt.style.width = "124px";
      txt.style.height = "92px";
      // txt.style.marginLeft = "48px";
    }

    console.log(document.querySelectorAll(".circle > #robotTXT"));
  }
}
  ...
  for (var i = 0; i < choiceQuestionArray.length; i++) {
    ...
    if (imgSrcArray) {
      ...
      if (isAnswerLength_3) {
        ...
        currentQuestion.style.textAlign = "right";
      }
    }
...
function puzzleCompareAnswer(dragObj, arrayNum) {
  ...
  for (i = 0; i < 1; i++) {
    ...
    (robotBgNone = dragObj.childNodes[1]), (dragObjAnswervalue = dragObj.getAttribute("answervalue")), (robotMax = Math.max.apply(null, gameManager.allImageArray).toString().length);
