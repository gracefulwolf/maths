...
function creatChoco() {
  ...
  for (var a = 0; a < ChocoGroup.childNodes.length; a++) {
    ...
    Chocos.addEventListener('mousedown', btnDown, false);
    // Chocos.addEventListener('touchstart', btnDown, false);
    Chocos.addEventListener('mouseup', btnUp, false);
    // Chocos.addEventListener('touchend', btnUp, false);
  }
}
...
function compareAnswer(dragObj) {
  ...
  if (
    gameManager.CURRENT_ANSWER[0][2] === parseInt(dragObj.getAttribute("answerValue")) &&
    gameManager.CURRENT_ANSWER[0][1] === parseInt(drag1.getAttribute("answerValue")) &&
    gameManager.CURRENT_ANSWER[0][0] === parseInt(drag2.getAttribute("answerValue"))
  ) {
    log("@ correct!!");
    gameOver(dragObj);
  } else {
