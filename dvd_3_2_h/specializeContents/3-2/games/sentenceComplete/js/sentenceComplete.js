...
function appendQuiz(buttonType, quizText, randomCount) {
  ...
  for (var i = 0; i < quizText.length; i++) {
    ...
    if (gameManager.QUIZ_ANSWER[0].toString().split("").length > 3) {
      choiceQuizText.setAttribute(
        "style",
        'background:url("images/centenceComplete_textBox_' + randomCount + '.png") no-repeat;' + "top :" + choiceTop + "px; left:" + choiceLeft + "px;"
      );
      ...
      choiceLeft += gameManager.quizText.length === 4 ? 282 : 440;

      if (gameManager.quizText.length === 3) {
        document.querySelector(".question").style.lineHeight = "60px";
      }
      // choiceLeft += 282;
    } else {
      choiceQuizText.setAttribute(
        "style",
        'background:url("images/sentenceComplete_textBox_s_' + randomCount + '.png") no-repeat;' + "top :" + choiceTop + "px; left:" + choiceLeft + "px;"
      );
      choiceQuizText.setAttribute("answerValue", gameManager.quizText[i]);
      choiceQuizText.innerHTML = gameManager.quizText[i];

      gameManager.quizPosition.push([choiceTop, choiceLeft]);
      choiceLeft += gameManager.quizText.length === 4 ? 220 : 350;
    }
