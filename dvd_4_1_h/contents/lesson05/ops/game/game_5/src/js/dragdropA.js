...
function boundingA(dragObj, x, y) {
	if (dropCompareA(dragObj, x, y)) {
		...
		var idx = dragObj.dataset.playIndex;
		window.stopSpriteAniByName(idx+"_correct");
...
/*
	var dragObjValue = dragObj.getAttribute("drag-ans")
	var dropAll = document.querySelectorAll(".dropArea[drag-ans='"+dragObjValue+"']");
	for (var i=0; i<dropAll.length; i++) {
		dropAll[i].classList.add("dropX");
	}
	*/
}
