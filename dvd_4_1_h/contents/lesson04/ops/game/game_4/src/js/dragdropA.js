...
function boundingA(dragObj, x, y) {
	if (dropCompareA(dragObj, x, y)) {
		...
		var dragDataAnswer = dragObj.getAttribute("drag-ans");
		var dragClones = document.querySelectorAll(".dragClone[drag-ans='"+dragDataAnswer+"']");
		
		dragClones.forEach(addClassView);
