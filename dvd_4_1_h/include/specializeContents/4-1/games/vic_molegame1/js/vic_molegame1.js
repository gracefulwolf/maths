function getIsFourMoles() {
  return gameManager.TOTAL_ANSWER_ARRAY.length === 9;
}
...
function queTxt() {
  var isFourMoles = getIsFourMoles();
  var lastNumber = gameManager.TOTAL_ANSWER_ARRAY[2];
  var firstOperatorValue = isFourMoles ? gameManager.TOTAL_ANSWER_ARRAY[7] : gameManager.TOTAL_ANSWER_ARRAY[6];
  var lastOperatorValue = isFourMoles ? gameManager.TOTAL_ANSWER_ARRAY[8] : gameManager.TOTAL_ANSWER_ARRAY[7];
  ...
  queText3.innerHTML = lastNumber;

  console.log(isFourMoles, firstOperatorValue, lastOperatorValue);

  switch (firstOperatorValue) {
    case "+":
      calulation = "plus";
      break;
    case "-":
...
function initMole() {
  ...
  for (var a = 0; a < len; a++) {
    ...
    moleTxt[a].innerHTML = gameManager.TOTAL_ANSWER_ARRAY[a + 3];
  ...
  if (isFourMoles) {
    mole_3 = document.querySelector("#mole_3");
    mole_3.setAttribute("answerValue", gameManager.TOTAL_ANSWER_ARRAY[6]);
  }
}
...
function createObject(index, top, left, parentObjSrc, moleHandSrc, moleHitSrc) {
  ...
  moleObjWrap.addEventListener('mousedown', eventCallback, false);
  // moleObjWrap.addEventListener('touchstart', eventCallback, false);
...
function compareAnswer(currentObj) {
  ...
  if (gameManager.selectArray.length === gameManager.CURRENT_ANSWER.length) {
    ...
    console.log(gameManager.selectArray);
    ...
    for (var i = 0; i < gameManager.selectArray.length; i++) {
      for (var z = 0; z < gameManager.CURRENT_ANSWER.length; z++) {
        if (gameManager.selectArray[i] === gameManager.CURRENT_ANSWER[z].toString()) {
          gameManager.compareAnswer.push(gameManager.selectArray[i]);
          // if (gameManager.CURRENT_ANSWER[z].toString() === gameManager.CURRENT_ANSWER[z + 1].toString()) gameManager.compareAnswer.splice(z, 1);
        }
      }
    }
