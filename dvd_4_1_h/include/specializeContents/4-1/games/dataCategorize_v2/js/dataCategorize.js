function initScene() {
  ...
  for (var i = 0; i < gameManager.QUIZ_OPTION.length - 3; i++) {
    ...
    for (var j = 0; j < gameManager.objCount[i]; j++) {
      ...
      productArray.sort(function () {
        return 0.5 - Math.random();
      });

      imgBox.style.transform = "translateY(-40px)";
    ...
    clickBtnBox.addEventListener('mousedown', onClickBtnBoxClick, false);
    // clickBtnBox.addEventListener('touchstart', onClickBtnBoxClick, false);
  }
  quizTableTextTop -= 24;
  for (var k = 0; k < gameManager.quizTableText.length; k++) {
    quizTableTextTop = quizTableTextTop + 62;
    quizTableText = createElement("div", quizTable, "quizTableText");
    quizTableText.innerText = gameManager.quizTableText[k];
    quizTableText.style.top = quizTableTextTop + "px";
    quizTableTextLeft;
...
function onClickBtnBoxClick(e) {
  ...
  if (gameManager.count[idx] < 10) {
    QS(".gauge_" + idx + "_" + (9 - gameManager.count[idx])).style.background = colorCode[idx];
...
function CheckButton() {
  ...
  btnDown = function (e) {
    e.preventDefault();
    checkBtn.style.background = 'url("../include/images/common/checkBtnRed_on.png") no-repeat';
    ...
    if (compareDap()) {
      ...
      gameReset();
      setTimeout(function () {
        checkBtn.style.background = 'url("../include/images/common/checkBtnRed.png") no-repeat';
      }, 100);
    }
  };
