function initScene() {
  ...
  if (gameManager.QUIZ_OPTION.length === 5 && gameManager.QUIZ_OPTION[4] !== "" && gameManager.QUIZ_OPTION[4] !== undefined) drawTitle();
  drawImgs();
  drawSymbols();
  machine();
}
...
function machine() {
  ...
  var configs = {
    ...
    doneCallBack: function () {
      ...
      for (var i = 0; i < gameManager.value.length; i++) {
        ...
        if (gameManager.value[i] === slotValues[gameManager.key[i]]) gameManager.dabCount++;
      }
  ...
  for (var i = 0; i < gameManager.quizObj.length; i++) {
    for (var j = 0; j < gameManager.quizObj[i][0][1].length; j++) {
      var slot = QS(".slot_" + i);
      if (gameManager.quizObj[i][0][0] == "image") {
        //이미지일 경우
        ...
        //숫자일 경우
        if (isNaN(gameManager.quizObj[i][0][1][j])) {
          //식일 때(문자형)
      ...
      } else if (gameManager.quizObj[i][0][0] == "text") {
        //문자일 경우
        slot.childNodes[0].childNodes[j].innerHTML = '<div class="slotRoom">' + gameManager.quizObj[i][0][1][j] + "</div>";
      }
    }
  ...
  document
    .querySelector(".doneBtn_on")
    .setAttribute(
      "style",
      "top : " +
        getRealOffsetTop(doneBtn) +
        "px; left :" +
        getRealOffsetLeft(doneBtn) +
        "px; background:url(../include/images/common/checkBtnRed_on.png) no-repeat; background-size:100% 100%;"
    );
}
