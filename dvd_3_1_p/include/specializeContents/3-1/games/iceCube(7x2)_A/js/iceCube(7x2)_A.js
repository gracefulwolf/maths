function initScene(val) {
	...
	for(var i in gameManager.quizConvertNumber){
  		...
  		// if(shapeName === "figure"){
  		// 	src = "../include/images/common/"+shapeName+"/"+(gameManager.quizConvertNumber[i])[1]+"_"+(gameManager.quizConvertNumber[i])[2]+"_"+(gameManager.quizConvertNumber[i])[3]+".png";
  		// } else {
  		// 	src = "../include/images/common/"+shapeName+"/"+(gameManager.quizConvertNumber[i])[1]+".png";
  		// }
  		src = "../include/images/common/"+shapeName+"/"+(gameManager.quizConvertNumber[i])[1]+".png";
...
function appendQuiz(buttonType, dragObjArray) {
	...
	for (var i = 0; i < dragObjArray.length; i++) {
		var dragObj = createElement('div',dragObjContainer,'drag_'+i),
			src = "../include/images/common/"+(dragObjArray[i])[0]+"/"+(dragObjArray[i])[1]+".png";
