...
function initScene() {
	...
	// if(gameManager.QUIZ_OPTION[0].constructor == Array){
	if (typeof gameManager.QUIZ_OPTION[0] === 'object') {
		...
		symbolBox(gameManager.QUIZ_OPTION[0][0], gameManager.QUIZ_OPTION[0][1], symbol, 100, 100);
	}
	else {
	 	currentAnswerText.innerHTML = "<span>" + '<span class="colorText" style="margin-left:-200px;">'+ gameManager.QUIZ_OPTION[0] + '</span>' + gameManager.QUIZ_OPTION[1] + "</span>";
	}
...
function symbolBox (folder, fileName, targetElement, width, height) {	
	...
	//  	symbolBox.src = '../include/images/common/' + folder + '/' + text + '_' + figure + '_' + colorRandom + '.png';
	//  } else {
	// 	symbolBox.src = '../include/images/common/' + folder + '/' + text + '_' + figure + '.png';
	// }

	symbolBox.src = '../include/images/common/' + folder + '/' + fileName + '.png';
...
function gameOver() {
	...
	// for (var i = 0; i < gameManager.selectBallArray.length; i++) {
		// for (var z = 0; z < gameManager.QUIZ_ANSWER.length; z++) {
			// if (gameManager.selectBallArray[i] == gameManager.QUIZ_ANSWER[z]) {
...
function createObject(index, top, left, eventCallback, balloonBoxSrc) {
	...
	balloonBox.addEventListener("mousedown", eventCallback, false);
	// balloonBox.addEventListener("touchstart", eventCallback, false);
