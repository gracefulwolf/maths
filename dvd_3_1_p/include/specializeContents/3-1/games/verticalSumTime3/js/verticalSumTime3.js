function initScene() {
  ...
  for (var i = 0, max = gameManager.QUIZ_TEXT.length; i < max; i++) {
    ...
    if (i !== 0 && i !== max - 1) {
      var num = gameManager.QUIZ_TEXT[i][0];
      var unit = gameManager.QUIZ_TEXT[i][1];

      // 분, 초인데 10 미만일 경우, 앞에 빈 text 넣기
      if (unit === '분' || unit === '초') {
        if (num.toString().length === 1) {
          num = '<span style="opacity: 0;">0</span>' + num;
        }
      }
      quizText.innerHTML = "<span class=num>" + num + "</span>" + unit;
