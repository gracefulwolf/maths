...
function gameOver(currentObj) {
  ...
  document
    ...
    .setAttribute(
      "style",
      "display:block; top:" +
        (currentObj.offsetTop + 20) +
        "px; left:" +
        (currentObj.offsetLeft + 80) +
        "px;"
    );
...
function createObject(index, top, left, eventCallback, parentObjSrc) {
  ...
  parentObj.addEventListener("mouseup", onParentObjUpEvent, false);
  // parentObj.addEventListener("touchend", onParentObjUpEvent, false);

  function onParentObjUpEvent() {
    document
      .querySelector("#numberPanel")
...
function incorrectAnimation(dragObj) {
  ...
  animate({
    ...
    step: function (delta) {
      ...
      dragObj.style.top =
        gameManager.choiceQuestionPosition[dragObjId[1]][0] + "px";
    },
  });
}
