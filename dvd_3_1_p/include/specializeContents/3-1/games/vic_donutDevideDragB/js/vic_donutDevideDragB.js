function initScene() {
  ...
  appendCircleElement("answerObject3Text", "answerObjectText", bgCanvas);
  for (var i = 0; i < 10; i++) {
    ...
    appendCircleElement("answertxt3", "Text", answerObject3Text);
    var answertxt3 = document.querySelectorAll("#answertxt3");
    answertxt3[i].innerHTML = i + 1;
  }
}
...
function initDonut(donutCounter) {
  ...
  questionTopWrp.setAttribute("style", "top: 9%; left:" + (1280 - 800) / 2 + "px");
...
function createObject(index, top, left, eventCallback, parentObjSrc) {
  var choiceTop = 25,
    verticalDonutLength = 5,
    choiceLeft = 95,
    baseLeft = 80,
    parentObj = document.createElement("img"),
    donutLength = gameManager.TOTAL_ANSWER_ARRAY[0];

  if (donutLength > 14) {
    choiceLeft = 74;
    baseLeft = 10;
    verticalDonutLength = 8;
  } else if (donutLength > 10) {
    choiceLeft = 88;
    baseLeft = 40;
    verticalDonutLength = 6;
  }
  ...
  if (index > verticalDonutLength - 1) {
    index = index - verticalDonutLength;
    choiceTop = choiceTop + 80;
    choiceLeft = choiceLeft * index + baseLeft;
    parentObj.setAttribute(
      "style",
      "padding-top: 6px; padding-bottom: 3px; top: " + choiceTop + "px; left:" + choiceLeft + "px; padding-left : " + left / 2.5 + "px; padding-right : " + left / 2.5 + "px;"
    );
  } else {
    choiceLeft = choiceLeft * index + baseLeft;
...
function donutNumTxt(dounuNumBox, answerObjecttxt, answertxt) {
  ...
  setTimeout(function () {
    ...
    for (var i = 1; i < 3; i++) {
      ...
      if (answerObjectText[i]) {
        answerObjectText[i].style.display = "none";
      }
    }
  }, 200);
}
...
function donutCompareAnswer(dragObj) {
  ...
  answerChildNode3 = parseInt(answerObj3.childNodes.length);

  if (gameManager.CURRENT_ANSWER[0] === answerChildNode1 && gameManager.CURRENT_ANSWER[0] === answerChildNode2 && gameManager.CURRENT_ANSWER[0] === answerChildNode3) {
