function initScene() {
  ...
  // for (var i in window) {
  //   if (window[i] !== null) console.log(i, ":", window[i]);
  // }
}
...
function drawLeafs() {
  ...
  for (var i = 0; i < 5; i++) {
    ...
    if (gameManager.QUIZ_OPTION[i] === "") {
      ...
      lotusleaf.setAttribute("answervalue", gameManager.QUIZ_ANSWER[areaIndex]);
...
function drawFrogs() {
  ...
  touchFrog.addEventListener('mouseup', onTouchFrogUp, false);
  // touchFrog.addEventListener('touchend', onTouchFrogUp, false);

  function onTouchFrogUp() {
...
function boundingCircle(dragObj, x, y) {
  ...
  if (dropCompare(dragObj, x, y)) {
    ...
    streamSound.setSound("../include/media/correct_2.mp3");
