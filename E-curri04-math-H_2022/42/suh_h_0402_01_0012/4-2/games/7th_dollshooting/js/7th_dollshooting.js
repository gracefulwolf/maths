function initScene() {
  ...
  bottom.setAttribute("style", "position: absolute; bottom: -30px; left: 0px; width: 100%; z-index: 11;");
...
function appendSelectQuestion(buttonType, choiceQuestionArray, imgSrcArray) {
  ...
  var isQuestionLetterOver_5 = gameManager.quizObj.some(function (obj) {
    return obj.toString().length >= 5;
  });

  for (var i = 0, quiz; i < choiceQuestionArray.length; i++) {
    quiz = gameManager.quizObj[i];
    ...
    if (imgSrcArray) {
      ...
      if (typeof quiz !== "object") {
        choiceQuestionGroup.innerHTML = quiz;
      } else {
        makeBunsu(quiz.toString(), choiceQuestionGroup);
        if (quiz[0] === 0 && quiz[1] < 10 && quiz[2] < 10) {
          choiceQuestionGroup.querySelector(".bunsuDiv").style.marginLeft = "-5px";
        }
      }
      ...
      // 5자리 이상일 경우 font size 40px로 줄임
      if (isQuestionLetterOver_5) {
        choiceQuestionGroup.style.fontSize = "40px";
      }
    } else {
...
function correctMotion(dragObj) {
  ...
  animalArray = [
    "images/dollshooting_" + objanimal + "_success_2.png",
    "images/dollshooting_" + objanimal + "_success_3.png",
    "images/dollshooting_" + objanimal + "_success_4.png",
    "images/dollshooting_" + objanimal + "_success_5.png",
    "images/dollshooting_" + objanimal + "_success_6.png",
    "images/dollshooting_" + objanimal + "_success_7.png",
  ];
  handArray = [
    "images/dollshooting_success_hand_1.png",
    "images/dollshooting_success_hand_2.png",
    "images/dollshooting_success_hand_3.png",
    "images/dollshooting_success_hand_4.png",
    "images/dollshooting_success_hand_5.png",
    "images/dollshooting_success_hand_0.png",
  ];
...
function clickCompareAnswer(clickObj) {
  ...
  console.log(gameManager.QUIZ_ANSWER, clickObjValue);
