function initScene() {
  ...
  if (gameManager.quizText[0] === "text") {
    questionBox.innerHTML = gameManager.quizText[1];
    questionBox.style.lineHeight = "90px";
  } else {
    var str = "../include/images/common/" + gameManager.quizText[0] + "/" + gameManager.quizText[1] + ".png";
  ...
  for (var i = 0; i < 2; i++) {
    ...
    if (i === 0) {
      ...
      top = content.offsetTop + 350;
    }
    dropArea.setAttribute("style", "top:" + top + "px; left:" + left + "px;");
    gameManager.dropArea.push(dropArea);
  }

  checkBtn.addEventListener('mouseup', checkFun, false);
  // checkBtn.addEventListener('touchend', checkFun, false);
}
...
function initObject(val) {
  ...
  for (var index = 1; index < val.length; index++) {
    var str = "../include/images/common/" + val[index][0] + "/" + val[index][1] + ".png";
