       ...
       var dragLength = $ts.getEl('.page3 .dropBox').length;
       ...
       var quizButtons = $ts.getEl('.page3 .quizButtons');
      ...
      function setNextButton(index) {
        console.log(index)
        if (index === 3 || index === 5) nextButton.classList.add('home');
        else hideNextButton();
      }
      
      function goPage() {
        var index = nextButton.getAttribute(ATTR_INDEX)-0;
        console.log(nextButton, index,pageLength,myType);
        if(index == 1 && myType == 3 ){
          index = 3;
      ...
      SETPAGE.page_2 = function() {
        var numInput = $ts.getEl('.page2 input');
      ...
      SETPAGE.page_3 = function() {
        btnToggle($ts.getEl('.page3 .input1 .quizButtons')[0], true);
        btnToggle($ts.getEl('.page3 .input2 .quizButtons')[0], true);
        ...
        if (document.querySelectorAll('.page3 .dropWrap1 .dropComplete').length > 0) {
        ...
        var numInput = $ts.getEl('.page3 .dragBox input');
        numInput.forEach(function(input) {
          input.addEventListener('keyup', onlyNumberInput);
          input.addEventListener('input',function(){
            var idx = parseInt(this.getAttribute('data-input-obj'));
            ...
            if (document.querySelectorAll('.page3 .dragBox input.inputComplete').length === 6) {
              ...
              document.querySelector('.page3 .dragBox .js-dragObj.left').setAttribute('data-answer-value', dragInputAnswer_1);
              document.querySelector('.page3 .dragBox .js-dragObj.right').setAttribute('data-answer-value', dragInputAnswer_2);
        ...
        var dropArea = document.querySelectorAll('.page3 .dropArea');
        dropArea.forEach(function(drop) {
          drop.innerHTML = '';
        })
        document.querySelector('.page3 .dropArea').innerHTML = '';
        
        var dragReset =  document.querySelector('.page3 .reset');
      ...
      SETPAGE.page_5 = function() {
        ...
        numCard = [];
        var numInput = $ts.getEl('.page5 .selfInput');
        var numView = $ts.getEl('.page5 .viewInput');
        var page5Btn = $ts.getEl('.page5 .quizButtons.answer[data-quiz-chain]')[0];

        page5Btn.classList.add('off');
        numInput.forEach(function(input) {
          input.addEventListener('keyup', onlyNumberInput);
          input.addEventListener('input', function(){
            var idx = parseInt(this.getAttribute('data-input-obj'));
            ...
            if(numInput.every(inputValueCheck)) {
              ...
              clickBtnControl(page5Btn, false);
            }
          });
        });
      ...
      window.$callBack.dragCorrect = function(DRAG) {
        ...
        var chk_box = document.querySelectorAll('.page3 [data-gr="'+gr+'"]');
        ...
        if (gr == 1) {
          var notDragNum = (document.querySelector('.page3 .left.dragObjComplete')) ? 1:0;
          ...
          if($ts.getEl('.dropWrap1 .dragObjComplete').length === 2) {
            btnToggle($ts.getEl('.page3 .input1 .quizButtons')[0], false);
          }
          return;
        } else if (gr == 2) {
          ...
          if (DRAG.droppedArea.element.getAttribute('data-answer-value') != dragValue) {
            ...
            if($ts.getEl('.dropWrap2 .dragObjComplete').length === 2) {
              btnToggle($ts.getEl('.page3 .input2 .quizButtons')[0], false);
            }
          }
      ...
      window.addEventListener('load', function() {
        ...
        oxQuizBtn.forEach(function (btn) {
          btn.addEventListener('click', function () {
            var oxTargetsPage = this.getAttribute('data-quiz-chain') === 'toggle_1, ox_1' ? '.page4 ' : '.page6 ';
        ...
        // var page3_answerButton = document.querySelectorAll('.page3 .js-buttons');
        // page3_answerButton.forEach(function (btn) {
        //   var input = document.querySelectorAll('.page3 input');
        //
