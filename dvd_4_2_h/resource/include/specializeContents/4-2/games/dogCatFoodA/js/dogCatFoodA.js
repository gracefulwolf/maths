...
function initBead(frogBg) {
  ...
  foodShadow.setAttribute("style", "position:absolute; top: 44%; left:355px;");

  frogBg.setAttribute("style", "top: 44%; left:355px;");
...
function gameOver(dragObj, x, y) {
  ...
  dragObj.setAttribute("style", "top:560px; left:360px;");
...
function appendChoiceQuestion(buttonType, choiceQuestionArray, imgSrcArray) {
  ...
  for (var i = 0; i < choiceQuestionArray.length; i++) {
    ...
    if (imgSrcArray) {
      ...
      var choiceQuestionValue = gameManager.choiceQuestion[i];
      var imgObjText = document.createElement("div");

      var choiceQuestionAllBox = document.createElement("div");
      ...
      if (typeof choiceQuestionValue === "string") {
        imgObjText.innerHTML = choiceQuestionValue;
        imgObjText.style.transform = "translate(-15px, 18px)";
        imgObjText.style.width = "160px";
      } else {
        var fountainText = document.createElement("div");
        var mixedFractionText = document.createElement("div");
        var fountainLine = document.createElement("div");
        ...
        mixedFractionText.innerHTML = choiceQuestionValue[0];
        ...
        imgObjText.innerHTML = choiceQuestionValue[1];

        if (choiceQuestionValue[0] === 0) {
          choiceQuestionAllBox.setAttribute("style", "position:absolute; top:-5px; left:60px; width:120px; height:125px;");
        } else {
          choiceQuestionAllBox.appendChild(mixedFractionText); //대분수
          choiceQuestionAllBox.setAttribute("style", "position:absolute; top:-5px; left:65px; width:120px; height:125px;");
        }
    ...
    currentQuestion.setAttribute("answerValue", choiceQuestionValue);
...
function frogMotioncorrect() {
  ...
  frogMotion.setAttribute("style", "top: 44%; left:355px; z-index: 0;");
...
function frogMotionIncorrect() {
  var frogBg = document.querySelector("#frogBg"),
    spriteArray1 = ["images/dogfood_1_fail_1.png", "images/dogfood_1_fail_2.png", "images/dogfood_1_fail_3.png"];
...
function dogCompareAnswer(dragObj) {
  ...
  var isAnswerValueBunsu = /,/g.test(answerValue);

  if (isAnswerValueBunsu) {
    answerValue = answerValue.split(",");
  }

  var isAnswered = isAnswerValueBunsu
    ? gameManager.CURRENT_ANSWER[0][0] === answerValue[0] && gameManager.CURRENT_ANSWER[0][1] === answerValue[1] && gameManager.CURRENT_ANSWER[0][2] === answerValue[2]
    : gameManager.CURRENT_ANSWER[0] === answerValue;
