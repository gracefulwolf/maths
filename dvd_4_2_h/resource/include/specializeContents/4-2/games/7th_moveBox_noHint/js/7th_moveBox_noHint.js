...
function initScene() {
  ...
  IS_TYPE_1 = /자료를 수집하여 분석하는 방법을 왼쪽부터 차례대로 옮겨 놓으세요./g.test(gameManager.QUIZ_OPTION[0]);
...
function drawDragObjs() {
  ...
  for (var i = 0; i < gameManager.QUIZ_OPTION.length - 1; i++) {
    ...
    if (gameManager.QUIZ_OPTION[i + 1] === "꺾은선 그래프로 자료 해석하기") {
      textDiv.style.letterSpacing = "-0.06em";
    }
...
function drawCheckBtn() {
  ...
  var checkBtn_on = createElement("div", checkBtnContainer, "checkBtn_on");
  var checkBtn = createElement("div", checkBtnContainer, "checkBtn");
  var left = 947;
